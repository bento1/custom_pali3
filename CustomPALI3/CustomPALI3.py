from CustomPALI3.CustomVit import CustomVit, CustomVitConfig
...
class CustomPALI3(PreTrainedModel):
    ...
    def generate(self,*arg):
        return self.model.generate(*arg)

class CustomPALI3_(torch.nn.Module):
    def __init__(self,model_name,dim,
        ...
        self.seq_len=seq_len
        self.vit_config=CustomVitConfig(dim=dim)
        self.vit_model=CustomVit(self.vit_config)
        ...
        if device is not None:
            ...
            self.pali_model.to(self.device)
            self.vit_model.to(self.device)
        
        if vit_fix:
            self.vit_model.requires_grad_(False)
        else:
            self.vit_model.requires_grad_(True)
            # self.vit_model.requires_grad(True)
    def forward(self, img, prompt, output, mask):
        
        img_embeds = self.vit_model(img)
        logits, loss = self.pali_model.forward(
            prompt, output, mask=mask, src_prepend_embeds=img_embeds
        )
        return logits, loss
    
    @torch.no_grad()
    def generate(self, image, prompt, mask=None, attn_mask=None):
        visual_tokens = self.vit_model(image)
        seq_out_start = torch.ones(1, 1).long().to(self.device)*32100
        result = self._generate(prompt, seq_out_start, visual_tokens, mask, attn_mask)
        return result

    def _generate(self,seq_in,seq_out_start,prepend_embeds,mask=None,attn_mask=None, **kwargs):
        
        encodings = self.pali_model.encoder(
            seq_in,
            mask=mask,
            attn_mask=attn_mask,
        ...
        return self.pali_model.decoder.generate(seq_out_start,self.seq_len,eos_token=1,context=encodings,context_mask=mask, **kwargs)
...
class RelativePositionBias(nn.Module):
    ...
    def forward(self, i, j):
        ...
        values = self.relative_attention_bias(rp_bucket.long())
